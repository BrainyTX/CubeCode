...
class MotorB:
    ...
    def __post_init__(self) -> None:
        ...
        self.unit_vectors = np.exp(1j * angles).astype(np.complex64)
    ...
    def step(self, state: GridState) -> None:
        ...
        resultant = np.zeros_like(phase_vectors, dtype=np.complex64)
        neighbour_counts = np.zeros(state.phase.shape, dtype=np.int16)
        ...
        coherence = (np.abs(resultant) / neighbour_counts).astype(np.float32)
        phase_angle = np.angle(resultant, deg=False)
        phase_angle = np.nan_to_num(phase_angle, nan=0.0)
        coupling = np.round(self.osc_cfg.kappa * phase_angle / (np.pi / 4.0)).astype(np.int32)

        base_increment = np.zeros_like(state.phase, dtype=np.int32)
        for r, coords in state.shell_lut.shells.items():
            inc = self._shell_increment(r)
            base_increment[tuple(coords.T)] = inc
        ...
        np.clip(state.amplitude, 0.0, None, out=state.amplitude)
