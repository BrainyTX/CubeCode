...
def enforce_mirror(state: GridState, config: MirrorConfig) -> None:
    ...
    for coords in state.mirror_groups.values():
        idx = state.coords_index(coords)
        values = to_numpy(state.values[idx])
        phases = to_numpy(state.phase[idx])
        ...
        state.values[idx] = target_val
        state.phase[idx] = target_phase
