...
from .config import BarrierConfig, GridConfig, MirrorConfig, OscillationConfig, ShellConfig
...
class GridState:
    ...
    def initialise(
        ...
        N = grid_cfg.N
        values = np.full((N, N, N), -1, dtype=np.int8)
        phase = np.zeros((N, N, N), dtype=np.uint8)
        amplitude = np.zeros((N, N, N), dtype=np.float32)
        fixed_mask = np.zeros((N, N, N), dtype=bool)
        mirror_id = np.full((N, N, N), -1, dtype=np.int16)
        front_active = np.zeros((N, N, N), dtype=bool)
        ...
        for idx, cells in mirror_map.items():
            arr = np.array(cells, dtype=np.int16)
