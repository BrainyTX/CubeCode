...
class Simulation:
    ...
    _prev_values: np.ndarray = field(init=False, repr=False)
    _prev_phase: np.ndarray = field(init=False, repr=False)
    ...
    def step(self) -> Dict[str, Any]:
        ...
        amp_mean = float(np.mean(self.state.amplitude))
        amp_max = float(np.max(self.state.amplitude))
    ...
    def snapshot(self) -> Dict[str, np.ndarray]:
        return {
            ...
            "phase": self.state.phase.copy(),
            "amplitude": self.state.amplitude.copy(),
        }
